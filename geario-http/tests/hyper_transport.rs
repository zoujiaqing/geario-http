...
fn pair() -> (GearioTransport<geario::io::Base>, GearioTransport<geario::io::Base>) {
    ...
    (
        GearioTransport::new(geario::net::from_tcp_stream(a, SharedCfg::new("CLIENT").into()).unwrap()),
        GearioTransport::new(geario::net::from_tcp_stream(b, SharedCfg::new("SERVER").into()).unwrap()),
    )
}
...
fn body(seed: u8) -> Bytes {
    (0..131_071).map(|i| (i as u8).wrapping_add(seed)).collect::<Vec<_>>().into()
}
    ...
    timeout(Seconds(5), async {
        ...
        geario::rt::spawn(async move {
            hyper::server::conn::http1::Builder::new()
                .serve_connection(server, service_fn(echo)).await.unwrap();
        });
        let (mut sender, conn) = hyper::client::conn::http1::handshake(client).await.unwrap();
        geario::rt::spawn(async move { conn.await.unwrap(); });
        for seed in 0..3 {
            eprintln!("seed {seed}");
            let expected = body(seed);
            let req = Request::post("/echo").header("host", "test")
                .body(Full::new(expected.clone())).unwrap();
            let resp = sender.send_request(req).await.unwrap();
            eprintln!("response {:?}", resp.headers());
            assert_eq!(resp.status(), 200);
            assert_eq!(resp.into_body().collect().await.unwrap().to_bytes(), expected);
            poll_fn(|cx| sender.poll_ready(cx)).await.unwrap();
        }
    }).await.expect("HTTP/1 transfer stalled");
}
...
async fn http2_concurrent_streams_cross_flow_control_window() {
    ...
    }).await.expect("HTTP/2 flow control stalled");
}
