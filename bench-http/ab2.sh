...
measure() {  # bin port
    ...
    BENCH_BODY_SIZE="$BODY" "$D/client" "127.0.0.1:$2" "$CONNS" 2 "$POST" >/dev/null 2>&1
    BENCH_BODY_SIZE="$BODY" "$D/client" "127.0.0.1:$2" "$CONNS" "$SECS" "$POST" \
        | awk '/^qps/{q=$2}/^p50/{p=$2}/^mismatches/{m=$2}/^errors/{e=$2}END{print q,p,m+e}'
    kill "$PID" 2>/dev/null; wait "$PID" 2>/dev/null; PID=""
}

echo "# ab $A vs $B body=$BODY conns=$CONNS post=$POST workers=$WORKERS rounds=$ROUNDS secs=$SECS"
